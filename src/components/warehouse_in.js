...
function WarehouseIn({state}) {
  ...
  const getCheckedValues = async() => {
    ...
    const { contract, web3 } = state;
        ...
        await contract.methods
          .updateStagetoWarehouseIn(checkedValues)
  ...
  data = detail
  .filter((item) => item.stage === "2").map((item, index) => ({
    product: item.name,
    ...
    return(
      ...
      <div className="container px-5 py-24 mx-auto">
        <div className="flex flex-col text-center w-full mb-20">
          <h1 className="sm:text-4xl text-3xl font-medium title-font mb-2 text-white">WAREHOUSE IN </h1>
          <p className="lg:w-2/3 mx-auto leading-relaxed text-base"> Products that was received at Warehouse</p>
        </div>
